...
def integrate():
    if (globals.get_value('PATTERN_ASCII')):
        os.system("ffmpeg -framerate " +
                  ...
                  "%05d.png" +
                  " -vcodec libx264 -pix_fmt yuv420p -s " + str(globals.get_value('wide')) + "*" + str(globals.get_value('high')) + " -r " +
                  str(globals.get_value('frames')) +
                  " -t " +
                  str(globals.get_value('dur')) +
                  " -y " +
                  globals.get_value('name_of_no_speed_video')
                  )
...
def merge():
    os.system(
        ...
        globals.get_value('name_of_final_video')
    )
